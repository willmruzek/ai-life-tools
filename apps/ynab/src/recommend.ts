...
export const main = async (): Promise<void> => {
  ...
  const sinceDateStr = getSinceDate();
  ...
  const prompt = dedent`
    You are a personal finance assistant and budget consultant. Analyze the following 3 months of bank transactions alongside the user's current YNAB category list and suggest concrete improvements to their category structure.
    ...
    output: Output.object({
      schema: z.object({
        summary: z
          .string()
          .describe(
            'Brief overall assessment of the current category structure',
          ),
        add: z.array(
          z.object({
  ...
  const recommendOutputDir = path.join(outputDir, 'recommend');
  await fs.mkdir(recommendOutputDir, { recursive: true });
